...
test('Create Boost', async ({ page }) => {
  ...
  const catalogs: IHydraResponse<ICatalog> = await (
    await page.waitForResponse(
      (response) =>
        response.url() === 'https://localhost/catalogs' && response.ok()
    )
  ).json()
  ...
  await expect(await nameInput.inputValue()).toBe('Test playwright')
  ...
  await catalogsCheckbox[0].click()
  catalogsTags = await page.getByTestId('localizedCatalogsTag').allInnerTexts()
  ...
  const successResponse = await (
    await page.waitForResponse(
      (response) => response.url() === "https://localhost/boosts" && response.ok()
    )
  ).json()

    console.log(successResponse.id)
  await expect(page).toHaveURL("https://localhost/fr/admin/merchandize/boost/grid")
  const link = await page.locator(`table td a[href="/fr/admin/merchandize/boost/edit?id=${successResponse.id}"]`)
  await expect(link).toBeVisible()
  await link.click()
  await expect(page).toHaveURL(`https://localhost/fr/admin/merchandize/boost/edit?id=${successResponse.id}`)
})

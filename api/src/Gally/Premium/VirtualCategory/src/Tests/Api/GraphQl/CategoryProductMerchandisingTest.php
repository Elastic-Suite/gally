...
namespace Gally\VirtualCategory\Tests\Api\GraphQl;

use Gally\Category\Tests\Api\GraphQl\CategoryProductMerchandisingTest as BaseCategoryProductMerchandisingTest;
use Gally\User\Constant\Role;

/**
 * Exactly the same tests as the parent class, but we changed the fixture's data:
 * - In fixtures/configurations.yaml only the category one is virtual now.
 * - In fixtures/product_documents.json we removed the category one id in all documents.
 *
 * With these changes we can test that the behavior between a no virtual catgory "one" and the virtual category "one" is the same,
 * event if we remove the category node for virtual categories.
 */

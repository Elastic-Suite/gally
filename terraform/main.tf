...
# Docker images (front  + php) are not buidable with the new archi because additional_context are not supported by the docker terraform provider
# manual action is required because the azure terraform provider do not support additional_port_mapping configuration (see varnish container app)
# fix all todos
...
resource "azurerm_container_registry" "cr" {
  #todo limiter l'utilisation du registre à des utilisateurs, car tout le monde peut pousser et récupérer des images
...
resource "azurerm_container_app" "pwa" {
  ...
  template {
    container {
      ...
      env {
        name  = "NEXT_PUBLIC_API_ROUTE_PREFIX"
        value = var.router.api_prefix
      }
      env {
...
resource "azurerm_container_app" "web" {
  ...
  template {
    ...
    container {
      ...
      env {
        name  = "ELASTICSEARCH_SSL_VERIFICATION"
        value = true #todo use variable ?
      }
      env {
        name  = "VARNISH_URL"
        value = "http://${local.containers.varnish.service}/"
      }
      volume_mounts {
        name = "php-static-files"
        path = "/app/public"
...
resource "azurerm_container_app" "varnish" {
  ...
  template {
    ...
    container {
      ...
      env {
        name  = "VARNISH_HTTP_PORTS"
        value = "${local.containers.varnish.target_port} ${local.containers.varnish.tcp_port}"
      }
      env {
  ...
  ingress {
    ...
    #todo ofr the moment we cannot add additional port mapping via terraform an issue is opened about this topic https://github.com/hashicorp/terraform-provider-azurerm/issues/23442
    # do it manually for the moment (add port additional port 8081 from azure portal)
    # additional_port_mapping {
    #       target_port = local.containers.varnish.tcp_port
    # }
    traffic_weight {
      percentage      = 100
      latest_revision = true
